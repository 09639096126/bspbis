...
WHERE sp.on_status = 0 AND sp.parent_id = '$parent_id'
...
$query_kids = "SELECT COUNT(*) FROM tbl_child WHERE parent_id = '$parent_id'";
$result_kids = mysqli_query($db, $query_kids);
...
$assistance_release = 0;

$query_release = "SELECT COUNT(*) FROM tbl_cash_assistance WHERE parent_id = '$parent_id'";
$result_release = mysqli_query($db, $query_release);
while ($row = mysqli_fetch_array($result_release)) {

    $assistance_release = $row[0];
}
...
$data['parents']            = $result_1;
$data['kids_count']         = $kids_count;
$data['assistance_release'] = $assistance_release;
